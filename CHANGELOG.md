...
- **Ctrl+V image paste on Wayland**: pasting an image only worked under X11 —
  on a Wayland session arboard fell back to the X11 backend and read an empty
  clipboard, so the paste silently did nothing. The `wayland-data-control`
  feature is now enabled so arboard reads the Wayland clipboard directly
  (falling back to X11 when unavailable).
